...
public:
  ...
  template <typename K, typename F> bool find_fn(const K &key, F fn) const {
    typename bucket::mapped_raw_type storage;
    if (read_value(key, *reinterpret_cast<mapped_type*>(&storage))) {
      fn(std::move(*reinterpret_cast<mapped_type*>(&storage)));
      return true;
    }
   ...
   * allow to modify the contents of the value if found. If used with Safe = false,
   * you should use atomic operations to update value, UB otherwise.
   *
   * @tparam K type of the key. This can be any type comparable with @c key_type
   * @tparam F type of the functor. It should implement the method
   ...
   * @return true if the key was found and functor invoked, false otherwise
   */
  template <operation_mode OPERATION_MODE = operation_mode::SAFE, typename K, typename F> bool 
  update_fn(const K &key, F fn) {
    const hash_value hv = hashed_key(key);
    TwoBuckets b = snapshot_and_lock_two<normal_mode>(hv);
    
    std::atomic_thread_fence(std::memory_order_release);
    
    const table_position pos = cuckoo_find<true>(key, hv.partial, b.i1, b.i2);
    if (pos.status == ok) {
      update_safely<OPERATION_MODE>(pos.bucket_->mapped(pos.slot), fn);
      return true;
    } else {
      b.unlock(false);
      return false;
   ...
   * key is found. The functor can not mutate the value, and should return @c true
  ...
  template <typename K, typename F> bool erase_fn(const K &key, F fn) {
    ...
    const table_position pos = cuckoo_find<true>(key, hv.partial, b.i1, b.i2);
    if (pos.status == ok) {
      if (fn(static_cast<const mapped_type&>(pos.bucket_->mapped(pos.slot)))) {
  ...
  bool uprase_fn(K&& key, F fn, Args&& ...val) {
    ...
    if (pos.status == ok) {
      ...
      if constexpr (OPERATION_MODE == operation_mode::SAFE) {
        mapped_type value = pos.bucket_->mapped(pos.slot);
        if (fn(value)) {
          del_from_bucket(*b.lock1, *pos.bucket_, pos.slot);
        } else {
          atomic_store_memcpy(&pos.bucket_->mapped(pos.slot), value);        
          b.unlock(false);
        }
      } else {
  ...
  bool upsert(K &&key, F fn, Args &&... val) {
    return uprase_fn<OPERATION_MODE>(
      std::forward<K>(key),
      [&fn](mapped_type &v) {
        fn(v);
        return false;
  ...
  template <typename K> bool find(const K &key, mapped_type &val) const {
    return find_fn(key, [&val](mapped_type&& v) mutable { val = v; });
  }
  ...
  template <typename K, typename... Args> bool insert(K&& key, Args&& ...val) {
    return upsert<operation_mode::UNSAFE>(std::forward<K>(key), [](mapped_type &) {},
                  std::forward<Args>(val)...);
  }
  ...
  template <typename K, typename V> bool insert_or_assign(K &&key, V &&val) {
    return upsert<operation_mode::SAFE>(std::forward<K>(key), [&val](mapped_type &m) { m = val; },
                  std::forward<V>(val));
  }

  template <typename K> bool insert_or_assign(K &&key, const mapped_type& val) {
  return upsert<operation_mode::UNSAFE>(std::forward<K>(key), 
      [&val](mapped_type& m) { 
        atomic_store_memcpy(&m, val);
      },
      val
  );
  }
  ...
  template <typename K> bool erase(const K &key) {
    return erase_fn(key, [](const mapped_type &) { return true; });
  }
...
private:
  ...
  using locks_t = lock_container<seqlock_t, Allocator>;
  ...
  using LockManager = std::unique_ptr<seqlock_t, LockDeleter<seqlock_t>>;
  ...
  public:
    ...
    TwoBuckets(size_type i1_, size_type i2_, locked_table_mode)
        : i1(i1_), i2(i2_) {}
    TwoBuckets(seqlock_t* lock1_, seqlock_t* lock2_, size_type i1_, size_type i2_, normal_mode)
        : i1(i1_), i2(i2_),
  ...
  inline void check_hashpower(size_type hp, seqlock_t &lock) const {
    if (hashpower() != hp) {
      lock.unlock();
  ...
  void migrate_lock(size_type l) const {
    ...
    for (size_type bucket_ind = l; bucket_ind < (buckets_.size() >> 1); bucket_ind += kMaxNumLocks) {
      ...
      [this, bucket_ind](size_type old_bucket_slot){
        buckets_.deoccupy(bucket_ind, old_bucket_slot);
      });
    }
  }
  ...
  seqlock_t* lock_and_rehash(size_type l) const {
    seqlock_t* lock = &locks_[l];
    ...
    if (!seqlock_t::is_migrated(lock_value)) {
      migrate_lock(l);
  ...
  LockManager lock_one(size_type hp, size_type i, normal_mode) const {
    seqlock_t* lock = lock_and_rehash<normal_mode>(lock_ind(i));
  ...
  TwoBuckets lock_two(size_type hp, size_type i1, size_type i2,
    ...
    seqlock_t* lock1 = lock_and_rehash<normal_mode>(l1);
    seqlock_t* lock2 = nullptr;
  ...
  std::pair<TwoBuckets, LockManager> lock_three(size_type hp, size_type i1,
    ...
    check_hashpower(hp, *lock_and_rehash<normal_mode>(l[0]));
    if (l[1] != l[0]) {
      lock_and_rehash<normal_mode>(l[1]);
    }
    if (l[2] != l[1]) {
      lock_and_rehash<normal_mode>(l[2]);
    }
  ...
  seqlock_t* read_and_rehash(seqlock_epoch_t& epoch, size_type l) const {
    seqlock_t* lock = &locks_[l];
    epoch = lock->get_epoch();

    if (seqlock_t::is_locked(epoch)) {
      return nullptr;
    }
    if (!seqlock_t::is_migrated(epoch)) {
      epoch = lock->lock();
      if (!seqlock_t::is_migrated(epoch)) {
        migrate_lock(l);
        lock->set_migrated(true);
  ...
  template <typename K>
  bool read_value(const K& key, mapped_type& value) const {
    const hash_value hv = hashed_key(key);
    ...
    while (true) {
      ...
      seqlock_t* lock1 = read_and_rehash(epoch1, l1);
      ...
      const table_position pos = cuckoo_find<false>(key, hv.partial, i1, i2);

      typename bucket::mapped_raw_type storage;
      if (pos.status == ok) {
        atomic_load_memcpy(reinterpret_cast<mapped_type*>(&storage), pos.bucket_->mapped(pos.slot));
      }

      std::atomic_thread_fence(std::memory_order_acquire);

      if (epoch1 == lock1->get_epoch(std::memory_order_relaxed) && 
          (l1 == l2 || epoch2 == lock2->get_epoch(std::memory_order_relaxed))) {
        if (pos.status == ok) {
          memcpy(&value, &storage, sizeof(mapped_type));
          return true;
        }
  ...
  template <bool Exclusive, typename K>
  table_position cuckoo_find(const K &key, const partial_t partial,
    ...
    int slot = try_read_from_bucket<Exclusive>(*bucket1, partial, key);
    if (slot != -1) {
      return table_position{bucket1, i1, static_cast<size_type>(slot), ok};
    }

    bucket* bucket2 = &buckets_[i2];
    slot = try_read_from_bucket<Exclusive>(*bucket2, partial, key);
    if (slot != -1) {
  ...
  int try_read_from_bucket(const bucket &b, const partial_t partial,
    ...
    for (int i = 0; i < static_cast<int>(slot_per_bucket()); ++i) {
      if constexpr (!Exclusive) {
        bool occupied;
        atomic_load_memcpy(&occupied, b.occupied(i));

        if (occupied) {
          if constexpr (!is_simple()) {
            partial_t b_partial;
            atomic_load_memcpy(&b_partial, b.partial(i));

            if (partial != b_partial) {
              continue;
            }
          }

          typename bucket::key_raw_type storage;
          atomic_load_memcpy(reinterpret_cast<typename bucket::key_type*>(&storage), b.key(i));
          if (key_eq()(*reinterpret_cast<typename bucket::key_type*>(&storage), key)) {
            return i;
          }
  ...
  table_position cuckoo_insert(const hash_value hv, TwoBuckets &b, K &key) {
    ...
    if (st == failure_under_expansion) {
      ...
      assert(TABLE_MODE() == locked_table_mode() ||
             !locks_[lock_ind(b.i1)].try_lock());
      assert(TABLE_MODE() == locked_table_mode() ||
             !locks_[lock_ind(b.i2)].try_lock());
      ...
      table_position pos = cuckoo_find<true>(key, hv.partial, b.i1, b.i2);
      if (pos.status == ok) {
  ...
  void add_to_bucket(seqlock_t& lock_, bucket& bucket_, const size_type slot,
  ...
  bool cuckoopath_move(const size_type hp, CuckooRecords &cuckoo_path,
    ...
    while (depth > 0) {
      ...
      buckets_.setKV(to.bucket, ts, fb.partial(fs), fb.key(fs), fb.mapped(fs));
      buckets_.deoccupy(from.bucket, fs);
      if (depth == 1) {
  ...
  cuckoo_status cuckoo_fast_double(int32_t current_hp) {
    const int32_t new_hp = current_hp + 1;
      ...
      for (size_type old_bucket_ind = 0; old_bucket_ind < hashsize(current_hp); 
        ...
        move_bucket(current_hp, old_bucket_ind, *it_old, *it_new,
          [this, &it_old, &it_lock_old, &it_lock_new](size_type old_bucket_slot) {
            buckets_.deoccupy(*it_old, old_bucket_slot);
            ...
            for (; i < end; ++i) {
              ...
              for (size_type j = 0; j < slot_per_bucket(); ++j) {
                if (bucket.occupied(j)) {
                  new_map.insert(bucket.key(j), bucket.mapped(j));
                }
              }
            }
          } catch (...) {
  ...
  // well.
  void del_from_bucket(seqlock_t& lock_, bucket& bucket_, const size_type slot) const {
    buckets_.deoccupy(bucket_, slot);
    --lock_.elem_counter();
  }

  void del_from_bucket(const size_type index, const size_type slot) const {
    buckets_.deoccupy(index, slot);
    --locks_[lock_ind(index)].elem_counter();
  }
  ...
  public:
    ...
    template <typename K> size_type erase(const K &key) {
      ...
      const table_position pos =
          map_.get().template cuckoo_find<true>(key, hv.partial, b.i1, b.i2);
      if (pos.status == ok) {
    ...
    template <typename K> size_type count(const K &key) const {
      ...
      const auto b =
          map_.get().template snapshot_and_lock_two<locked_table_mode>(hv);
      return map_.get().template cuckoo_find<true>(key, hv.partial, b.i1, b.i2).status == ok
                 ? 1
                 : 0;
    }
