...
private:
  ...
  static ResultT find_value(size_type i, F func) {
    static constexpr uint16_t type_size_ind = (sizeof(size_type) << 3);
...
protected:
  static constexpr size_type size_by_index(int32_t  index) {
    return index == 0 ? 1 : hashsize(index - 1);
  }
  ...
  void change_size(int32_t new_hp) {
    ...
    if (hp < new_hp) {
      ...
      for (int32_t i = hp; i > new_hp; --i) {
        destroy_and_deallocate(data_[i],i);
      }
    }
  }
...
public:
  ...
  void destroy_and_deallocate(data_pointer& pointer, size_type index) noexcept {
    size_type cur_size = size_by_index(index);
    for (size_type j = 0; j < cur_size; ++j) {
      traits::destroy(data_allocator_, &pointer[j]);
    }

    data_allocator_.deallocate(pointer, cur_size);
    pointer = nullptr;
  }
