...
TEST(BucketContainer, CopyAssignmentWithPropagate) {
  ...
  ASSERT_EQ(tc2[0].partial(0), 2);
  ASSERT_EQ(*tc2[0].key(0), 10);
  ASSERT_EQ(tc2[0].key(0).use_count(), 2);
  ASSERT_EQ(tc2[0].mapped(0), 5);
...
TEST(BucketContainer, MoveAssignmentNoPropagateEqual) {
  ...
  ASSERT_EQ(tc2[0].partial(0), 2);
  ASSERT_EQ(*tc2[0].key(0), 10);
  ASSERT_EQ(tc2[0].key(0).use_count(), 1);
  ASSERT_EQ(tc2[0].mapped(0), 5);
  ASSERT_FALSE(tc2[1].occupied(0));
  ASSERT_EQ(tc2.get_allocator().id, 5);
}
...
TEST(BucketContainer, SwapNoPropagate) {
  ...
  ASSERT_EQ(tc[1].key(0).use_count(), 1);
...
TEST(BucketContainer, SetKVWithThrowingTypeMaintainsStrongGuarantee) {
  ...
  ASSERT_THROW(container.setKV(0, 1, 0, 0, 0), std::runtime_error);
  ExceptionInt::do_throw = false;

  ASSERT_TRUE(container[0].occupied(0));
  ASSERT_EQ(container[0].key(0).x, 10);
  ASSERT_EQ(container[0].mapped(0), 20);

  ASSERT_FALSE(container[0].occupied(1));
}

TEST(BucketContainer, CopyAssignmentWithThrowingTypeIsDestroyedProperly) {
  ExceptionContainer container(0, ExceptionContainer::allocator_type());
  container.setKV(0, 0, 0, ExceptionInt(10), 20);
  ExceptionContainer other(0, ExceptionContainer::allocator_type());

  ExceptionInt::do_throw = true;
  ASSERT_THROW(other = container, std::runtime_error);
  ExceptionInt::do_throw = false;
}

TEST(BucketContainer, CopyDestroyedBucketsContainer) {
  std::allocator<value_type> a;
  TestingContainer<decltype(a)> bc(2, a);
  ASSERT_FALSE(bc.is_deallocated());
  bc.clear_and_deallocate();
  ASSERT_TRUE(bc.is_deallocated());
  auto bc2 = bc;
  ASSERT_TRUE(bc.is_deallocated());
  ASSERT_TRUE(bc2.is_deallocated());
  ASSERT_EQ(bc.size(), bc2.size());
  ASSERT_EQ(bc.get_allocator(), bc2.get_allocator());
}
