...
TEST(MaximumHashpowerInitializedToDefault, MaximumHashPower) {
  IntIntTable tbl;
  ASSERT_EQ(tbl.maximum_hashpower(), seqlock_lib::cuckoo::NO_MAXIMUM_HASHPOWER);
}
...
TEST(CapsAnyExpansion, MaximumHashPower) {
  ...
  ASSERT_THROW(tbl.insert(2 * tbl.slot_per_bucket(), 0),
               seqlock_lib::cuckoo::maximum_hashpower_exceeded);
  ASSERT_THROW(tbl.rehash(2), seqlock_lib::cuckoo::maximum_hashpower_exceeded);
...
TEST(NoMaximumHashPower, MaximumHashPower) {
  ...
  ASSERT_THROW(tbl.rehash(3), seqlock_lib::cuckoo::maximum_hashpower_exceeded);

  tbl.maximum_hashpower(seqlock_lib::cuckoo::NO_MAXIMUM_HASHPOWER);
  tbl.rehash(10);
